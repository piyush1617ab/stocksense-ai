...
The frontend (under `frontend/`) is wired with TanStack Query + a centralized API client
([`frontend/src/lib/api.ts`](frontend/src/lib/api.ts)). To go live, point `VITE_API_URL` at your backend, set
`VITE_USE_MOCKS=false` in `frontend/.env.local`, and implement the endpoints below.
...
## Type contracts
See [`frontend/src/types/api.ts`](frontend/src/types/api.ts) for full Zod schemas. They double as TypeScript types
and runtime validators.

## Switching from mocks → real backend
1. Set `VITE_USE_MOCKS=false` in `frontend/.env.local`.
2. Set `VITE_API_URL=https://your-backend.example.com`.
3. From the repo root, run `npm run dev` (or `cd frontend && npm run dev`). The UI is unchanged; data now comes from your API.
